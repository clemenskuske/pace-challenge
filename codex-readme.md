...
The exact-track scoring gate is implemented by `scripts/score-exact.py`, using the official exact-track rule: every correctly solved instance is worth 1 point, unsolved/timeouts are worth 0, and any infeasible or known non-optimal answer disqualifies the score. The current stored score is 0 out of 150 in `scores/current-score.json` on the larger `data/benchmarks/exact_public.tsv` benchmark, with 150 public exact instances and a deliberately tiny per-instance timeout of 0.05s. Public exact optimum sizes are not published in the instance files, so rows without `expected_size` can only score 0 until a certified optimum is added. Pushes to `main` are allowed only when the freshly computed score is higher than the score stored on the remote `main` commit, unless `scores/reset.json` changed to intentionally reset the gate. The committed `scores/current-score.json` must always match the freshly computed result.
